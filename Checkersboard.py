...
def calc_heuristic(board, player):
    ...
    if player.player == "R":
        ...
        for x, y in player_pieces:
            ...
            position_sum += position_val * y
        # Add random bit so its no the same always

        rand_list = [random.random() for _ in range(0, 10)]
        places = random.random(0,9)

        rand_bit = rand_list[places]
...
def game(game_board):
    while game_board.won == "":
        ...
        if game_board.turn == 1:
            ...
            m = root_min_max(rootr, 2)
            print "Move generated for Min-Max: ", m, "\n"
        ...
        if game_board.won == "":
            ...
            m = root_min_max(rootb, 5)
            print "Move generated by Min-Max: ",m, "\n"
